...
public class BasicBinaryTree <X extends Comparable <X>> {
    ...
    public void add(X item){
        ...
        //if this is the first item on the tree, set it as root.
        if(root == null) {
            this.root = node;
            System.out.println("Set root: " + node.getItem());
            this.size++;
        }
        //otherwise we need to insert the item into the tree using the binary tree insert algorithim
        else {
    ...
    public boolean delete(X item) {
        ...
        if (currentNode != null) {
            ...
            //if the node to delete has only a right child, remove it in the hierarcy
    ...
    private void insert(Node parent, Node child){
        ...
        if(child.getItem().compareTo(parent.getItem()) < 0) {
            ...
            //otherwise we need to call insert again and test for left of right (recursion)
            else {
                insert(parent.getLeft(), child);
            }
        }
        //if child is greater than the parent, it goe on the right.
            ...
            //otherwise we need to call the insert again and test for left or right (recursion)
            else {
                insert(parent.getRight(), child);
            }
        }

        //if the parent and child happens to be equal, we don't do anything
